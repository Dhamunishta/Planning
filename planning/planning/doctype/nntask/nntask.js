...
cur_frm.fields_dict['assign_to'].grid.get_field('members').get_query = function(
		doc, cdt, cd) {
	//alert(doc.tasklist)
...
frappe.ui.form.on("NNTask", "refresh", function(frm) {
	cur_frm.add_custom_button("Check In / Out List", function() {
		frappe.set_route("", "check-in-out");
	})
})
