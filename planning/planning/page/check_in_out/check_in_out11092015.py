...
def getTask(doctype):
	data = []
	user_name = frappe.session.user
	select_task = frappe.db.sql("select name,parent,members,employee_name,parenttype,creation from `tabNNAssign` where  close_status=0 and parenttype=%s and employee_name=%s order by creation DESC", (doctype, user_name))
	if(select_task):
		i = 1;
		values = "";
		for select_task_list in select_task:
			...
			creation = select_task_list[5];			
			format = "%d-%m-%Y %H:%M:%S"
			ct = creation.strftime(format)
			...
			rows = [project_name] + [milestone] + [task_list_name] + [task_name] + [employee_name] + [check_status] + [check_status_name] + [duration] + [worked_time] + [assign_name] + [ct]
			data.append(rows)
			i = i + 1;
		return data
...
def close_task(assign_name=None,):
	status = frappe.db.sql("""Update `tabNNAssign` set close_status=1 where name=%s""", (assign_name))
