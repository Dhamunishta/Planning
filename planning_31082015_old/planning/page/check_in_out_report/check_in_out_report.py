...
def report_in_out(date1=None, date2=None, project=None, employee=None):
    ...
    for num in range(0, no_days):
        ...
        task_lists = frappe.db.sql("select t1.name as name,t1.duration as duration,t1.tasklist as tasklist,t1.expected_start_date as date from `tabNNTask` t1,`tabNNAssign` t2 where  t1.name=t2.parent and t2.employee_name=%s  and date(t1.expected_start_date)=%s " + filter, (user_name, date_loop), as_dict=True)
